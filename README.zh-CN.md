...
* [√] **分布式幂等控制**：接入 Redis 实现分布式锁控制与过期自愈机制。
* [√] **全链路异步演进**：基于 asyncio/httpx 实现全链路异步非阻塞工具链调用。
* [ ] **长效偏好记忆感知**：引入 LangGraph Store 机制，实现异步非阻塞的用户偏好（Preference）长效记忆。
* [ ] **前后端分离聊天交互 (Web-UI)**：基于 FastAPI (Async) 搭建后端接口，利用 WebSocket / SSE (Server-Sent Events) 技术实现类似 ChatGPT 的流式（Streaming）打字机对话效果。
